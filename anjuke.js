...
function getNotch(slicePng, bgPng){
  ...
  var width = (notchRect.width + 2 * notchRect.x) > sliceMat.cols ? sliceMat.cols : (notchRect.width + 2 * notchRect.x)
  const sliceMatchMat = sliceGrayMat.getRegion(new cv.Rect(0, notchRect.y, width, notchRect.height))

  cv.imwrite("./anjuke/slice.jpg", sliceMat)
  cv.imwrite("./anjuke/slice_gray.jpg", sliceGrayMat)
  cv.imwrite("./anjuke/slice_match.jpg", sliceMatchMat)
  ...
  cv.imwrite("./anjuke/bg.jpg", bgMat)
  cv.imwrite("./anjuke/bg_gray.jpg", bgGrayMat)
  return x;
}
...
async function verify() {
  ...
  const url = 'https://www.anjuke.com/captcha-verify/?callback=shield&from=antispam'
  await driver.get(url)

  //截取原始验证码图片
  const bgImg = await driver.findElement(webdriver.By.css('.dvc-captcha__bgImg'))
  ...
  cv.imwrite("./anjuke/origin.jpg", originMat)

  //截取slice图片
  await driver.executeScript(`document.querySelector('.dvc-captcha__bgImg').style.display = 'none'`)
  const sliceImg = await driver.findElement(webdriver.By.css('.dvc-captcha__puzzleImg'))
  const slicePng = await sliceImg.takeScreenshot()
  
  //截取background图片
  await driver.executeScript(`document.querySelector('.dvc-captcha__puzzleImg').style.display = 'none'`)
  await driver.executeScript(`document.querySelector('.dvc-captcha__bgImg').style.display = 'block'`)
  ...
  await driver.executeScript(`document.querySelector('.dvc-captcha__puzzleImg').style.display = 'block'`)
  const slideBtn = await driver.findElement(webdriver.By.className('dvc-slider__handler'))
  ...
  if(url == curUrl){
    console.log("验证失败")
    var tag = uuid.v1()
    cv.imwrite("./anjuke/failImg2/" + tag + "_slice.jpg", sliceMat)
    cv.imwrite("./anjuke/failImg2/" + tag + "_bg.jpg", bgMat)
  }else{
    console.log("验证成功")
  }
  await sleep(1000)
...
!async function(){
  ...
  var succ = 0
  for(var i = 0; i < num; i++){
    console.log(">>>>>>>>>>>>> start " + (i + 1) + " attempt, succss:" + succ + ", fail:" + (i - succ)  + ">>>>>>>>>>>>>>>>>>>")
    await sleep(500)
    try{
